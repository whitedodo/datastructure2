/*
	...
	Director - ....... Jeong / OOOOOOOOOOOOOOOOO교 / Business & Computer Science
...
// Specialize Float Linked_Create 생성
template <>
LinkedList<std::string>* JApp<std::string>::Linked_Create(const int n, LinkedList<std::string>* pList, const int SetRandType){
	...
	// 초기화
	srand((unsigned int)time(J_NULL)); // Seed 값 사용
...
/// Specialize Double Linked_Create 생성
template <>
...
void JLand::About(){
	...
	std::cout << "--------------------------------------------" << std::endl;
	std::cout << "Rabbit.white(정OO)" << std::endl;
	std::cout << "OOOOOOOOOOOOOOOOOOO교" << std::endl;
